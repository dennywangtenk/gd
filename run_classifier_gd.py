...
class GlassdoorProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_examples(os.path.join(data_dir, "gd_train.csv"))

    def get_dev_examples(self, data_dir):
        return self._create_examples(os.path.join(data_dir, "gd_test.csv"), is_test=True)

    def get_labels(self):
        """See base class."""
        return ["good","bad"]

    def _create_examples(self, input_file, is_test=False):
        """Creates examples for the training and dev sets."""
        examples = []
        with tf.gfile.Open(input_file) as f:
            reader = csv.reader(f)
            for i, line in enumerate(reader):
                ...
                cons_txt = line[0].replace('"', ' ').replace('\\"', '"').replace('\n', ' ')
                point = float(str(line[1]))
                label = 'good'
                if point >= 3.5:
                    label = 'good'
                else:
                ...
                pros_txt = line[2].replace('"', ' ').replace('\\"', '"').replace('\n', ' ')

                examples.append(
                        InputExample(guid="unused_id", text_a=cons_txt, text_b=None, label=label))
                #examples.append(
                        #InputExample(guid="unused_id", text_a=pros_txt, text_b=None, label="pros"))
        return examples
